...
class ChatroomRouter {
    ...
    getChatroom(req, res) {
        ...
        return this.chatroomService.getChatroom(chatroom_id)
    ...
    addChatroom(req,res){
        ...
        return this.chatroomService.getChatroom(chatroom,user_id)
            .then(chatroom => res.send(chatroom))
            .catch(err => console.log(err))
    }
    getChatroomUsers(req, res) {
        let chatroom_id = req.params.id
        return this.chatroomService.getChatroomUsers(chatroom_id)
            .then(chatroom => res.send(chatroom))
            .catch(err => console.log(err))
    }
    addChatRecord(req, res) {
        ...
        return this.chatroomService.addChatRecord(new_chatRecord, chatroom_id, user_id)
            .then(chatRecord => res.send(chatRecord))
            .catch(err => console.log(err))
    }
    getRoomAllChatRecords(req, res) {
        let chatroom_id = req.params.id
        return this.chatroomService.getRoomAllChatRecords(chatroom_id)
