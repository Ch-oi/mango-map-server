...
class BlogRouter {
    ...
    addBlog(req, res) {
        ...
        return this.blogService.addBlog(new_blog)
            .then(newBlog => {res.send(newBlog)})
            .catch(err => {console.log(err)})
    }
    addBlogImages(req, res) {
        let urls = [...req.body.urls]
        let blog_id = req.params.id
        return this.blogService.addBlogImages(urls, blog_id)
            .then(newImgs => {res.send(newImgs)})
            .catch(err => {console.log(err)})
    }

    addBlogComment(req, res) {
        let comment = { ...req.body }
        return this.blogService.addBlogComment(comment)
    ...
    addBlogCategories(req, res) {
        ...
        let blog_id = req.params.id
        return this.blogService.addBlogCategories(cates_id, blog_id)
            .then(newBlogCates => {res.send(newBlogCates)})
            .catch(err =>{ console.log(err)})
    }
    ...
    addCategories(req, res) {
        ...
        return this.blogService.addCategories(new_categories)
            .then(new_categories => {res.send(new_categories)})
            .catch(err => {console.log(err)})
    }
}
