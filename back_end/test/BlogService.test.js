...
describe('BlogService testing with blogservice', () => {
    ...
    test('get one particular BlogCategories ', () => {
        return blogService.getBlogCategories(1)
            .then((results) => {
                expect(results.length).toBe(3)
    ...
    test('get one particular BlogComments ', () => {
        return blogService.getBlogComments(1)
            .then((results) => {
                expect(results.length).toBe(3)
    ...
    test('add new blogComment', () => {
        return blogService.addBlogComment(new_comment)
            .then(res => {
    ...
    test('add new categories', () => {
        return blogService.addCategories(new_category)
            .then(res => {
                expect(res.length).toBe(2);
    ...
    test('add new blog categories', () => {
        return blogService.addCategoriesBlogs(categories_id,blog_id)
            .then(res => {
                expect(res.length).toBe(2);
