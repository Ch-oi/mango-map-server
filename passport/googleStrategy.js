    ...
    passport.use(new GoogleStrategy({
        ...
        async (accessToken, refreshToken, profile, done) => {
            try {
                ...
                let registeredWithGoogle = await knex('users').where('google_ID', profile.id)
                ...
                if (registeredWithGoogle) {
                    ...
                    let updateUser = await knex('users').update('google_ID', profile.id).where('email', profile.emails[0].value)
