...
class UserRouter {
    ...
    getUserDistricts(req, res) {
        let user_id = req.params.id
        return this.userService.getUserDistricts(user_id)
            .then(userDistrictsBlogs => {res.send(userDistrictsBlogs)})
            .catch(err => {console.log(err)})
    }
    getUserFavBlogs(req, res) {
        let user_id = req.params.id
        return this.userService.getUserFavBlogs(user_id)
            .then(favBlogs => {res.send(favBlogs)})
            .catch(err => {console.log(err)})
    }
    getUserChatrooms(req, res) {
        let user_id = req.params.id
        return this.userService.getUserChatrooms(user_id)
    ...
    getUserChatroomRecords(req, res) {
        ...
        return this.userService.getUserChatroomRecords(user_id,chatroom_id)
    ...
    addUser(req, res) {
        ...
        return this.userService.addUser(new_user)
    ...
    addUserFavBlog(req,res){
        ...
        return this.userService.addUserFavBlog(user_id,blog_id)
    ...
    addUserDistrict(req,res){
        ...
        return this.userService.addUserDistrict(user_id,district_id)
            .then(results => {res.send(results)})
            .catch(err => {console.log(err)}) 
    }
}
